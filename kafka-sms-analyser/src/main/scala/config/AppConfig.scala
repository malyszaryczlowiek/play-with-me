...
object AppConfig {
  ...
  case class KafkaConfig(servers: String, fileStore: String)
   ...
   */
  def smsInputTopicName: String = config.getString(s"kafka-broker.topic.sms-input-topic.name")


  /**
   * topic where we keep all numbers with phishing service turned off.
   */
  def userStatusTopicName: String = config.getString(s"kafka-broker.topic.user-status-topic.name")


  /**
   * topic where we keep information about uri confidence status
   */
  def uriConfidenceTopicName: String = config.getString(s"kafka-broker.topic.uri-confidence-level-topic.name")


  /**
   * topic where we redirect sms with many uri links
   */
  def smsWithManyUriTopicName: String = config.getString(s"kafka-broker.topic.sms-with-many-uri-topic.name")
