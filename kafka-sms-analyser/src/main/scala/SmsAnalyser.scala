...
//    properties.put(SslConfigs.SSL_KEY_PASSWORD_CONFIG,           KafkaSSLConfig.SSL_KEY_PASSWORD_CONFIG)
    ...
    val uriWithConfidenceOrNull: KStream[Uri, ConfidenceOrNull]  = uriStream.leftJoin( uriTable )(
      (uri,urii) => uri,
    ...
    val nonEmptyUriList: KStream[Sms, UriList] = checkedUriDeleted.filter((sms, uriList) => uriList.list.nonEmpty, Named.as("non_empty_uri_list"))
    val emptyUriList: KStream[Sms, UriList] = checkedUriDeleted.filter((sms, uriList) => uriList.list.isEmpty, Named.as("empty_uri_list"))
